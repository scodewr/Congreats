...
@ApplicationScoped
public class GetDiscoveryFeedUseCase {
    ...
    public PageResult<RecognitionView> execute(int page, int size) {
        List<Recognition> recognitions = recognitionRepository.findRecent(page, size);
        long total = recognitionRepository.countAll();
    ...
    private RecognitionView toView(Recognition r, Map<UUID, User> users, Map<UUID, Category> categories) {
        ...
        String recognizerPhoto = profileRepository.findByUserId(r.recognizerId())
                .map(Profile::photoUrl).orElse(null);
        String recognizedPhoto = profileRepository.findByUserId(r.recognizedId())
                .map(Profile::photoUrl).orElse(null);
        ...
        return new RecognitionView(
                ...
                new RecognitionView.PersonRef(
                        ...
                        recognizer != null ? recognizer.name() : "Usuário",
                        recognizerPhoto),
                new RecognitionView.PersonRef(
                        recognized != null ? recognized.id() : r.recognizedId(),
                        recognized != null ? recognized.name() : "Usuário",
                        recognizedPhoto),
