...
@ApplicationScoped
public class GetWorkspaceRecognitionsUseCase {
    ...
    public PageResult<RecognitionView> execute(UUID requesterId, UUID workspaceId, int page, int size) {
        if (!workspaceRepository.findById(workspaceId).isPresent())
            throw new NotFoundException("Workspace não encontrado");
        if (!workspaceRepository.isMember(workspaceId, requesterId))
            throw new ForbiddenException("Acesso restrito a membros do workspace");

        List<Recognition> recognitions = recognitionRepository.findByWorkspaceId(workspaceId, page, size);
        long total = recognitionRepository.countByWorkspaceId(workspaceId);
        ...
        List<RecognitionView> views = recognitions.stream()
                .map(r -> toView(r, userCache, categoryCache))
                .toList();

        return new PageResult<>(views, total, page, size);
    }

    private RecognitionView toView(Recognition r, Map<UUID, User> users, Map<UUID, Category> categories) {
        ...
        String recognizerPhoto = profileRepository.findByUserId(r.recognizerId()).map(Profile::photoUrl).orElse(null);
        String recognizedPhoto  = profileRepository.findByUserId(r.recognizedId()).map(Profile::photoUrl).orElse(null);

        return new RecognitionView(
                r.id(),
                new RecognitionView.PersonRef(
                        recognizer != null ? recognizer.id() : r.recognizerId(),
                        recognizer != null ? recognizer.name() : "Usuário", recognizerPhoto),
                new RecognitionView.PersonRef(
                        recognized != null ? recognized.id() : r.recognizedId(),
                        recognized != null ? recognized.name() : "Usuário", recognizedPhoto),
